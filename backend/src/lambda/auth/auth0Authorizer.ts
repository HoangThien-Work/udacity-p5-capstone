...
async function verifyToken(authHeader: string): Promise<JwtPayload> {
  ...
  const headerKid: string = jwt.header.kid;
  const publicKey: string = await getCert(headerKid);
...
async function getCert(headerKid: string): Promise<string> {
  logger.info('Get cert, header kid', headerKid)
  var getJwks = await Axios.get(jwksUrl);
